...
/*
操作数栈，底层还是用数组实现，但对外要暴露的接口是栈的特性
包含的操作和局部变量表类似
操作数栈的大小是编译期已经确定的，保存在code属性中，所以可以用Slot数组实现
但要和 LocalVars区分开，本地变量表按索引访问，操作数栈是用数组模拟的栈，方法栈是用单向链表模拟的栈
fixBug: 统一说明：在操作数栈中，涉及到引用的操作：popRef和popSlot
之前的做法是将ref pop出来后，将数组中当前位置设置为null
...
public class OperandStack {
    // 初始值为0，在运行中，代表当前栈顶的index，用完需要size++
    private int size;
    private Slot[] slots;
